...
- **Due Management**: Comprehensive tracking system for all customer bills with "Due" payment status. Automatically identifies orders with due payments (both single payment and split payment scenarios). Features include summary statistics (Total Due Amount, Total Orders, Unique Customers), flexible filtering by search (order number, customer name, phone), date range (Today, This Week, This Month, Custom Date), and month selection. Filters work in combination for precise data analysis. Displays detailed order information including customer details, due amounts, and payment methods in a tabular format. Includes action buttons for each bill: View (shows complete order details with error-handling for payment splits), Edit (navigates to sales page using wouter for SPA routing), and Delete (with confirmation dialog and proper cache invalidation).
