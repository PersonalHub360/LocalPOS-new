...
- **Reporting System**: Comprehensive reports for Sales, Inventory, Payments, Discounts, Refunds, and Staff Performance with various date filters and export options. Payment-specific reports (Payment Gateway and Payments History) are available through a separate "Payment Report" filter beside the date range.
  - **Report Filters Layout**: Horizontal filter bar with Report Type, Date Range, and Payment Report dropdowns. Payment Gateway and Payments History options are in the separate Payment Report dropdown (not in main Report Type). Additional payment filters (Payment Method, Payment Status) appear conditionally based on selected report types.
