...
# Deployment script for BFC POS System
...
# Configuration
APP_DIR="/var/www/bfcpos"
DEPLOY_USER="nodejs"
BACKUP_DIR="/var/backups/bfcpos"
SERVICE_NAME="bfcpos"
PORT=7000
...
if [ -d "$APP_DIR" ] && [ "$(ls -A $APP_DIR)" ]; then
    BACKUP_FILE="$BACKUP_DIR/bfcpos-$(date +%Y%m%d-%H%M%S).tar.gz"
    echo "Creating backup: $BACKUP_FILE"
    sudo tar -czf "$BACKUP_FILE" -C "$APP_DIR" . || true
    
    # Keep only last 5 backups
    sudo find "$BACKUP_DIR" -name "bfcpos-*.tar.gz" -type f | sort -r | tail -n +6 | xargs -r sudo rm -f
fi
...
if [ -f "$APP_DIR/start.sh" ]; then
    sudo cp "$APP_DIR/start.sh" /tmp/start-bfc.sh.backup
fi

sudo tar -xzf /tmp/bfcpos-deploy.tar.gz --overwrite

# Restore start.sh if it was backed up
if [ -f "/tmp/start-bfc.sh.backup" ]; then
    sudo mv /tmp/start-bfc.sh.backup "$APP_DIR/start.sh"
    sudo chmod +x "$APP_DIR/start.sh"
fi

# Rename ecosystem config if needed
if [ -f "$APP_DIR/ecosystem-bfc.config.cjs" ] && [ ! -f "$APP_DIR/ecosystem.config.cjs" ]; then
    sudo mv "$APP_DIR/ecosystem-bfc.config.cjs" "$APP_DIR/ecosystem.config.cjs"
fi
...
sudo cp "$APP_DIR/bfcpos.service" /etc/systemd/system/bfcpos.service
...
if [ ! -f "$APP_DIR/.env.production" ]; then
    ...
    echo "  - DATABASE_URL"
    echo "  - SESSION_SECRET"
    echo "  - PORT (optional, defaults to 7000)"
    exit 1
fi
...
if [ $ATTEMPT -eq $MAX_ATTEMPTS ]; then
    ...
    # Rollback
    LATEST_BACKUP=$(sudo find "$BACKUP_DIR" -name "bfcpos-*.tar.gz" -type f | sort -r | head -1)
